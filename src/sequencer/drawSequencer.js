...
export function drawSequencer() {
    ...
    switch (project.instrumentsArray[project.activeInstrument].gridResolution) {
        ...
        case 8:
            numberOfCells = project.instrumentsArray[project.activeInstrument].patterns[project.instrumentsArray[project.activeInstrument].activePattern][0][0] / 8;
            break;
        case 16:
            numberOfCells = project.instrumentsArray[project.activeInstrument].patterns[project.instrumentsArray[project.activeInstrument].activePattern][0][0] / 4;
            break;

        case 32:
            numberOfCells = project.instrumentsArray[project.activeInstrument].patterns[project.instrumentsArray[project.activeInstrument].activePattern][0][0] / 2;
            break;

        case 64:
            numberOfCells = project.instrumentsArray[project.activeInstrument].patterns[project.instrumentsArray[project.activeInstrument].activePattern][0][0];
            break;
    }


    this.cellWidth = baseCellWidth * project.instrumentsArray[project.activeInstrument].gridResolutionMultiplier * project.instrumentsArray[project.activeInstrument].zoomLevelX - 1;
    this.cellHeight = baseCellHeight * project.instrumentsArray[project.activeInstrument].zoomLevelY;
    rowWidth = (this.cellWidth + 1) * numberOfCells;
    timeLineCellWidth = baseCellWidth * 0.25 * project.instrumentsArray[project.activeInstrument].zoomLevelX;
    ...
    if (project.instrumentsArray[project.activeInstrument].type === 'drums') {
        ...
        scrollContainerHeight = ((this.cellHeight + 1) * (project.octaves * project.notes.length)) + 23;
    }
    ...
    if (project.instrumentsArray[project.activeInstrument].type === 'drums') {
        ...
        for (let i = 0; i < project.instrumentsArray[project.activeInstrument].kits[project.instrumentsArray[project.activeInstrument].activeKit].drums.length || i < 32; i++) {
    ...
    let rulerCells = '';
    let pickerCells = '';
    let tempStepLength = project.instrumentsArray[project.activeInstrument].patterns[project.instrumentsArray[project.activeInstrument].activePattern][0][0];
        ...
        if (i % 16 === 0 || i === 1) {
            if (i !== tempStepLength) {
                ...
                for (let j = 1; j <= 16; j++) {
                    ...
                    if (j === 16) {
                        rulerCells += `<div class="timeLineCell" id="timeLineRulerCell${timeLineCellIndex}" style="border-right:1px solid white; width:${timeLineCellWidth - 1}px"></div>`;
                    } else if (j % 4 === 0 && j !== 16) {
                        rulerCells += `<div class="timeLineCell" id="timeLineRulerCell${timeLineCellIndex}" style="width:${timeLineCellWidth}px"><div class="timeLineTickNote" style="width:${timeLineCellWidth - 1}px"></div></div>`;
                    } else if (j % 2 === 0 && j !== 16) {
                        rulerCells += `<div class="timeLineCell" id="timeLineRulerCell${timeLineCellIndex}" style="width:${timeLineCellWidth}px"><div class="timeLineTickHalf" style="width:${timeLineCellWidth - 1}px"></div></div>`;
                    } else {
                    ...
                    if (j === 16) {
                        pickerCells += `<div class="timeLinePickerCell" id="timeLinePickerCell${timeLineCellIndex}" style="border-right:1px solid white; width:${timeLineCellWidth - 1}px"></div>`;
                    } else if (j === 1) {
    ...
    if (project.instrumentsArray[project.activeInstrument].type === 'drums') {
        ...
        if (project.instrumentsArray[project.activeInstrument].kits[project.instrumentsArray[project.activeInstrument].activeKit].drums.length < 32) {

            horLineCount = 32;

        } else {

            horLineCount = project.instrumentsArray[project.activeInstrument].kits[project.instrumentsArray[project.activeInstrument].activeKit].drums.length;
    ...
    if (project.instrumentsArray[project.activeInstrument].type === 'drums') {
        ...
        for (let i = 0; i < project.instrumentsArray[project.activeInstrument].kits[project.instrumentsArray[project.activeInstrument].activeKit].drums.length; i++) {
    ...
    this.html += `<svg id="sequencerCellsSvg" width="${rowWidth}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 ${rowWidth} ${y2 - 1}"><rect x="0" y="0" width="${rowWidth}" height="${y2}" fill="#363636" />${this.cellsHtml}${this.activeCellsHtml}</svg>`;
    this.cellsHtml = '';
    ...
    let autoDownArrowSvg = mainInterface.downArrow.replace(/ReplaceName/g, `showAutomationIconSvg`);
    autoDownArrowSvg = autoDownArrowSvg.replace(/width="6"/g, `width="10"`)


    let automationCells = this.drawSequencerAutomation(rowWidth, tempStepLength, scrollContainerHeight);
    ...
    if (project.instrumentsArray[project.activeInstrument].automation.automationVisiblePattern === false) {

        document.getElementById('sequencerAutomationCellsContent').className = 'sequencerAutomationCellsContentHidden';
        document.getElementById('showAutomationIcon').style.transform = 'rotate(-90deg)';
    ...
    let tempAlienCells = document.getElementsByClassName('sequencerCellActiveAlien');
    ...
    document.getElementById('sequencerAutomationCells').addEventListener('scroll', function (e) {
        ...
        document.getElementById('scrollContainer').scrollLeft = e.target.scrollLeft;

    });

    document.getElementById('sequencerAutomationToggle').addEventListener('mouseup', function () {

        if (project.instrumentsArray[project.activeInstrument].automation.automationVisiblePattern) {

            project.instrumentsArray[project.activeInstrument].automation.automationVisiblePattern = false;
            document.getElementById('sequencerAutomationCellsContent').className = 'sequencerAutomationCellsContentHidden';
            document.getElementById('showAutomationIcon').style.transform = 'rotate(-90deg)';

        } else {

            project.instrumentsArray[project.activeInstrument].automation.automationVisiblePattern = true;
            document.getElementById('sequencerAutomationCellsContent').className = 'sequencerAutomationCellsContent';
            document.getElementById('showAutomationIcon').style.transform = 'rotate(0)';
